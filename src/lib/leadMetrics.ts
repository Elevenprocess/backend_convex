...
export const LEAD_METRICS: Record<LeadMetricKey, {
  ...
  leads: { label: 'Prospects', color: '#00A8E8', format: 'count', valueOf: (p) => p.leads },
  calls: { label: 'Appels', color: '#3DA86A', format: 'count', valueOf: (p) => p.calls },
  rdv: { label: 'RDV', color: '#3DC6FF', format: 'count', valueOf: (p) => p.rdv },
  signed: { label: 'Ventes', color: '#0075A8', format: 'count', valueOf: (p) => p.signed },
