...
const PALETTE = ['#0075A8', '#3DC6FF', '#00A8E8', '#6B7C8C', '#3DA86A', '#3D5DC8', '#7B96EB', '#2D1A8C', '#4C7A8C', '#5E6F7D']
      ...
      {expandedReason && (
        <div className="glass-card p-6 border-or/40">
          ...
          {(() => {
            ...
            return <BarList entries={entry.subs} total={entry.count} color="#3DC6FF" />
          })()}
        </div>
      )}
      ...
      <div className="grid grid-cols-12 gap-6">
        <div className="glass-card p-6 col-span-12 xl:col-span-6">
          ...
          {objectionsOvercome.length === 0 ? (
            ...
            <BarList entries={objectionsOvercome} total={objectionsOvercome.reduce((s, e) => s + e.count, 0)} color="#00A8E8" />
          )}
        </div>
        <div className="glass-card p-6 col-span-12 xl:col-span-6">
          <div className="flex items-center justify-between mb-4">
          ...
          {objectionsBlocking.length === 0 ? (
            ...
            <BarList entries={objectionsBlocking} total={objectionsBlocking.reduce((s, e) => s + e.count, 0)} color="#0075A8" />
          )}
        </div>
      </div>
    </section>
