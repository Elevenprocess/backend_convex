...
test("règles de création : nom, scopes, expiration, admin only", async () => {
  ...
  // Sans scope : clé valide pour la surface Hermes historique, mais aucun accès /api/v1.
  const bare = await admin.mutation(api.apiTokens.create, { name: "legacy" });
...
// ─── Compat surface Hermes historique (requireServiceKey) ───────────────────
