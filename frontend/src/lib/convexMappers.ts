...
export function mapConvexLead(doc: ConvexLeadDoc): LeadResponse {
  return {
    ...
    // Agrégats read-side : présents quand le doc vient de leads:listEnriched
    // (jauges appels 4/jour et 11 jours) ; neutres sur leads:get/list bruts.
    assignedSetterIds: doc.assignedSetterIds ?? (doc.setterId ? [doc.setterId] : []),
    latestRdvAt: iso(doc.latestRdvAt),
    latestRdvStatus: (doc.latestRdvStatus ?? null) as LeadResponse['latestRdvStatus'],
    latestRdvCommercialId: doc.latestRdvCommercialId ?? null,
    jauge11Jours: null,
    ...
    joursSansContact: doc.joursSansContact ?? null,
    joursRelance: doc.joursRelance ?? null,
    callCount: doc.callCount ?? 0,
    callsToday: doc.callsToday ?? 0,
    nextCallbackAt: iso(doc.nextCallbackAt),
    callbackSetAt: iso(doc.callbackSetAt),
    firstCallUnderFiveMin: null,
    monetaryValue: null,
    ghlStageName: null,
    ghlPipelineId: null,
    lostReason: null,
    lastStageChangeAt: iso(doc.lastStageChangeAt),
    daysSinceLastStageChange: doc.daysSinceLastStageChange ?? null,
    arrivalAt: iso(doc.arrivalAt),
    transferredAt: iso(doc.transferredAt),
    hasDevis: doc.hasDevis,
    latestDevisAt: iso(doc.latestDevisAt),
    hasDebrief: doc.hasDebrief,
    latestDebriefAt: iso(doc.latestDebriefAt),
    delivrabiliteStatus: (doc.delivrabiliteStatus ?? null) as LeadResponse['delivrabiliteStatus'],
  }
}
